...
{
    public class FirmaService : Service<Firmalar>, IFirmaService
    {
        private readonly IFirmaRepository _main_repository;
        ...
        public FirmaService(IGenericRepository<Firmalar> repository, IUnitOfWork unitOfWork, IMapper mapper, IFirmaRepository main_repository, IHttpContextAccessor httpContextAccessor, IMemberService memberService) : base(repository, unitOfWork)
        {
            _mapper = mapper;
        ...
        public async Task<FirmaReturnModel> TableDataAsync(DataTableModel table)
        {

            var query = _main_repository.GetAll(false)
                .OrderBy(i => i.OrderNo).AsQueryable();

            //if(activeUserRoleId == 2)
            //{
            //    query.Where(i => i.)
            //}
            ...
            {
                query = query.Where(i => i.tr_Baslik.Contains(table.search.value) || i.CreatedDate.ToString().Contains(table.search.value));
            }
